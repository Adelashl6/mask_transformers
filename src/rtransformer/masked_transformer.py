...
class MTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        vfeat_size = config.get('video_feature_size') # 1024
        d_model = config.get('hidden_size')  # 768
        d_hidden = config.get('intermediate_size')  # 768
        n_encoder_layers = config.get('num_encoder_hidden_layers') # 4
        n_decoder_layers = config.get('num_decoder_hidden_layers')  # 2
        n_heads = config.get('num_attention_heads')  # 12
        drop_ratio = config.get('hidden_dropout_prob')  # 0.1
        self.vocab_size = config.get('vocab_size')
        self.encoder = Encoder(vfeat_size, d_model, d_hidden, n_encoder_layers,
                               n_heads, drop_ratio)
        self.decoder = Decoder(d_model, d_hidden, self.vocab_size,
                               n_decoder_layers, n_heads, drop_ratio)
        self.loss_func = LabelSmoothingLoss(config.get('label_smoothing'), config.get('vocab_size'), ignore_index=-1) \
            if "label_smoothing" in config.keys() and config.get('label_smoothing') > 0 else nn.CrossEntropyLoss(ignore_index=-1)
